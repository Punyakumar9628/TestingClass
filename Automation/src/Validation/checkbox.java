...
public class checkbox {
    ...
    public static void checkboxValidations() {
        ...
        try {
            // ✅ Set the path of ChromeDriver (update with your location)
            //

            oBrowser = new ChromeDriver();
            oBrowser.manage().window().maximize();
            oBrowser.navigate().to("file:///C:/JAVA/html/checkbox.html");
            Thread.sleep(2000);
            ...
            //4 & 5. Unselect and verify
            System.out.println("*Unselect the checkbox/radio & verify*");
            oBrowser.findElement(By.xpath("//input[@id='chk1_id']")).click();
            Thread.sleep(2000);
            if (oBrowser.findElement(By.xpath("//input[@id='chk1_id']")).isSelected()) {
                System.out.println("Failed to unselect the Checkbox");
            } else {
                System.out.println("The checkbox is unselected successfully");
            }
            ...
            Thread.sleep(2000);
            oEles.get(1).click();
            if (oEles.get(1).isSelected()) {
                System.out.println("Second checkbox is selected");
            } else {
                System.out.println("Failed to select the second checkbox");
            }
            Thread.sleep(2000);
